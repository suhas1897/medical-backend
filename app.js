...
// Fetch user history (appointments & medical records)
app.get("/history/:userId", async (req, res) => {
  try {
    const { userId } = req.params;

    // Find appointments where user is doctor or patient
    const appointments = await Appointment.find({
      $or: [{ doctorId: userId }, { patientId: userId }],
    });

    // Fetch medical records (only for patients)
    const medicalRecords = await MedicalRecord.find({  $or: [{ doctorId: userId }, { patientId: userId }], });

    res.json({ appointments, medicalRecords });
  } catch (error) {
...
app.post("/register", async (req, res) => {
  try {
    ...
    if (image) {
      ...
      imageFilename = Date.now() + "-profile.jpg";
...
// app.post("/appointment", async (req, res) => {
//   try {
//     const { doctorId, doctorName,patientId,patientName, appointmentDate, timeSlot,doctorImage,patientImage } = req.body;
...
//      // Construct full URL for the image if it exists
//      const dimageUrl = doctorImage
//      ? `${req.protocol}://${req.get("host")}/uploads/${doctorImage}`
//      : null;

//       // Construct full URL for the image if it exists
//     const pimageUrl = patientImage 
//     ? `${req.protocol}://${req.get("host")}/uploads/${patientImage}`
//     : null;


//     // Create new appointment
//     const appointment = new Appointment({ doctorId,doctorName, patientId,patientName, appointmentDate, timeSlot,doctorImage:dimageUrl,patientImage:pimageUrl });
//     await appointment.save();
...
app.post("/appointment", async (req, res) => {
  try {
    ...
    if (!doctorId || !doctorName || !patientId || !patientName || !appointmentDate || !timeSlot) {
    ...
    // Extract only the image file name from the full URL if provided
    const dimageFileName = doctorImage ? doctorImage.split("/").pop() : null;
    const pimageFileName = patientImage ? patientImage.split("/").pop() : null;

    // Create new appointment with only the image file names
    const appointment = new Appointment({
      doctorId,
      doctorName,
      patientId,
      ...
      appointmentDate,
      timeSlot,
      doctorImage: dimageFileName,
      patientImage: pimageFileName,
    });
